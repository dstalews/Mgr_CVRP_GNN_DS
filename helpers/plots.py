...
def plot_loss_curve(train_loss, val_loss, test_loss, config, name):
...
def plot_cvrp_heatmap(p, x_coord, W_val, W_pred, title="default"):
    """
    Helper function to plot predicted cvrp tours with edge strength denoting confidence of prediction.
...
def plot_predictions(x_nodes_coord, x_edges, x_edges_values, y_edges, y_pred_edges, num_plots=3):
    """
    Plots groundtruth cvrp tour vs. predicted tours (with beamsearch).
    ...
    for f_idx, idx in enumerate(np.random.choice(len(y), num_plots, replace=False)):
        ...
        plot_cvrp(plt1, x_coord.squeeze(), W.squeeze(), W_val.squeeze(), W_target.squeeze(), 'Groundtruth') 
        plt2 = f.add_subplot(132)
        plot_cvrp_heatmap(plt2, x_coord.squeeze(), W_val.squeeze(), W_sol_probs.squeeze(), 'Prediction Heatmap')
        plt.show()

def plot_cvrp(p, x_coord, W, W_val, W_target, title="default"):
    """
    Helper function to plot cvrp tours.
...
def plot_two_routes(x_coord, W_val, route1, route2, file_name):

    f = plt.figure(0, figsize=(15, 5))
    plt1 = f.add_subplot(131)
    plot_route(plt1, x_coord, W_val.squeeze(), route1, 'OR Tools') 
    plt2 = f.add_subplot(132)
    plot_route(plt2, x_coord, W_val.squeeze(), route2, 'BeamSearch')
    #plt.show()
    plt.savefig(file_name)

    
def plot_route(p, x_coord, W_val, route, title="default"):
    """
    Helper function to plot cvrp tours.
    ...
    def _route_to_node_pairs(W):
        """Helper function to convert route into pairs of adjacent nodes.
        """
        pairs = []
        for i in range(1,len(W)):
            if W[i] != W[i-1]:
                pairs.append((W[i-1],W[i]))
        return pairs
    
    G = nx.DiGraph(W_val)
    pos = dict(zip(range(len(x_coord)), x_coord))
    target_pairs = _route_to_node_pairs(route)
    colors = ['g'] + ['b'] * (len(x_coord) - 1)  # Green for 0th node, blue for others
    nx.draw_networkx_nodes(G, pos, node_color=colors, node_size=50)
    nx.draw_networkx_edges(G, pos, edgelist=target_pairs, alpha=1, width=1, edge_color='r')
    p.set_title(title)
