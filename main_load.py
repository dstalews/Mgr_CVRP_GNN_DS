...
i_tab = range(0,num_samples)
for sl in sim_limits:
    for k in beamwidth:
        i = 0
        beam_tab = []
        for td in test_datasets:
            t = time.time()
            #new = cvrproute.CVRPRoute(len(td[2]),copy.deepcopy(td[0]))
            #route, length = beamSearch.get_best_route_bs(td[1],td[2],k,new, True)
            route2, length, sel_time, exp_time, sim_time, backp_time = cvrpboard.play_game(td[0],td[1],td[2],1,'B',k,sl)
            end_time = time.time() - t
            beam_tab.append([td[3], beamSearch.score_length(td[1], td[3]), route2, length, k, end_time, sel_time, exp_time, sim_time, backp_time])
            ...
            #plots.plot_two_routes(td[4], td[5], td[3], route, file_name=f'./plots/CVRP_{len(set(route))}_{j}_{k}.png')
        df = pd.DataFrame(beam_tab,
                    index=i_tab, columns=['OR Tools trasa', 'OR Tools długość', 'BMCTS trasa', 'BMCTS długość', 'beamwidth', 'time(s)', 'sel_time', 'exp_time', 'sim_time', 'backp_time'])
        df.to_excel(f'./plots_and_data/data_BMCTS_{num_nodes}_{k}_{sl}.xlsx', sheet_name=f'data_BMCTS_{num_nodes}_{k}_{sl}')